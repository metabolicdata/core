...
class MetabolicAppIT extends AnyFunSuite
  ...
  test("WITH statement in Select") {
    ...
    val testingConfig = Config(
      ...
      Environment("", EngineMode.Batch, "", false,"dbName","",region, Option.empty, Option.empty, Option.empty, false, true)
  ...
  test("Change Schema") {
    ...
    val testingConfig = Config(
      ...
      Defaults(ConfigFactory.load()),
      Environment("", EngineMode.Batch, "",  false, "dbName", "", region, Option.empty, Option.empty, Option.empty)
