...
class DateFieldUpToReaderTest extends AnyFunSuite
  with DataFrameSuiteBase
  with SharedSparkContext
  with BeforeAndAfterAll{

  test("Filters < 2020 rows in") {
    ...
    val outputDF = inputDF
      .transform(new DateFieldUpToReader("ts", new DateTime(new Timestamp(1590999363000L).getTime)).filter())
